...
<head>
    ...
    <title>Create Food Product - GetFed Healthy Food</title>
    <style>
...
<body>
    ...
    <main class="create-product-container">
        ...
        if (isset($_POST['submit'])) {
            ...
            // Prepare statement for inserting food product details
            $stmt = mysqli_prepare($conn, "INSERT INTO food_products (name, description, price, category) VALUES (?, ?, ?, ?)");
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($image_error === 0 && in_array($image_type, $allowed_types)) {
                    ...
                    if (move_uploaded_file($image_tmp_name, $target)) {
                        // Update image URL in the food_products table
                        $stmt_image = mysqli_prepare($conn, "UPDATE food_products SET image_url = ? WHERE id = ?");
                        mysqli_stmt_bind_param($stmt_image, "si", $image_name, $product_id);
